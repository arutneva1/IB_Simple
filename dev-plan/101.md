...
- Note any warnings when the market is closed with `prefer_rth=true`.

**What success looks like:**
- Messages **"Submitting batch..."** and **"Done."** appear.
- TWS → Orders shows each symbol as **"Filled"** or **"Submitted."**
- If the market is closed and `prefer_rth=true`, the app warns and stops before sending orders.
...
- Real app: two **timestamped CSVs** appear.
  - **Plan report**: symbol, target %, current %, drift %/$, action, qty.
  - **Results report**: symbol, qty, fill price, status, etc.
