...
def test_confirm_per_account_missing_price(tmp_path):
    cfg = AppConfig(
        ibkr=IBKR(host="localhost", port=4001, client_id=1, read_only=False),
        models=Models(smurf=0.5, badass=0.3, gltr=0.2),
        rebalance=Rebalance(
...
def test_confirm_per_account_missing_result_entry(tmp_path):
    cfg = AppConfig(
        ibkr=IBKR(host="localhost", port=4001, client_id=1, read_only=False),
        models=Models(smurf=0.5, badass=0.3, gltr=0.2),
        rebalance=Rebalance(
    ...
    with pytest.raises(IBKRError, match="Missing fill result"):
        asyncio.run(
            confirm_per_account(
                plan,
                args,
    ...
    row = appended[0]
    assert row["status"] == "failed"
    assert "Missing fill result" in row["error"]
